...
func execTestDeletionRules(i int, testCase struct {
	...
	defer cleanup(s3Client, bucketName, testCase.object, function, args, startTime, true)
	...
	if err != nil {
		failureLog(function, args, startTime, "", "Put LifecycleConfiguration failed", err).Fatal()
		return
	}
	...
	if err != nil {
		failureLog(function, args, startTime, "", "PUT expected to succeed but failed", err).Fatal()
		return
	}
	...
	if err != nil {
		...
		if !ok {
			failureLog(function, args, startTime, "", "Unexpected non aws error on GetObject", err).Fatal()
			return
		}
		if testCase.expDeletion && aerr.Code() != s3.ErrCodeNoSuchKey || !testCase.expDeletion {
			failureLog(function, args, startTime, "", "Unexpected aws error on GetObject", err).Fatal()
			return
		}
	}
	...
	if err != nil {
		failureLog(function, args, startTime, "", "Expected to return data but failed", err).Fatal()
		return
	}
	result.Body.Close()

	if string(body) != "my content 1" {
		failureLog(function, args, startTime, "", "Unexpected body content", err).Fatal()
		return
	}
